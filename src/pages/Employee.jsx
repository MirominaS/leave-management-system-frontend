...
const Employee = () => {
    ...
    const [clearLeave, setClearLeave] = useState(() => {
        const saved = localStorage.getItem("clearedLeaves");
    ...
    const handleClear = (leaveId) => {
        const updated = [...clearLeave, leaveId];

        setClearLeave(updated)

        localStorage.setItem("clearedLeaves",JSON.stringify(updated))
    };
  ...
  return (
    <div className="h-screen w-full bg-[#F6E7BC] flex flex-col overflow-hidden font-sans text-[#0B2D72]">
            <nav className="h-16 w-full bg-[#0B2D72] text-white 
                px-8 flex justify-between items-center shadow-lg border-b-4 border-[#0AC4E0] shrink-0">
                <div className="flex items-center gap-6">
                    <h2 className="text-lg font-black tracking-tighter">ABC Company</h2>
                    <span className="hidden sm:block text-[9px] font-black uppercase tracking-[0.2em] text-[#0AC4E0]">
                        Employee Portal
                    </span>
                    {/* Check userRole is 4 (Manager) or 6 (Admin) */}
                    {privilegedRoles.includes(userRole) && (
                    <div className="flex gap-2">
                        <button 
                            onClick={() => navigate('/dashboard')}
                            className="bg-[#0AC4E0] text-[#0B2D72] text-[10px] 
                            font-black uppercase px-4 py-2 rounded-lg hover:bg-white 
                            hover:scale-105 transition-all shadow-[0_0_15px_rgba(10,196,224,0.4)] 
                ...
                <button 
                    className="text-[10px] font-black uppercase tracking-widest bg-white/10 
            ...
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="w-full max-w-xl bg-white rounded-3xl shadow-xl 
                    border-b-[10px] border-[#0B2D72] flex flex-col max-h-[90vh]">
                    <div className="p-6 md:p-8 overflow-y-auto">
                        ...
                        {latestLeave && (
                            <div className={`mb-6 p-4 rounded-xl border-2 border-dotted 
                                    flex justify-between items-center ${
                                latestLeave.status === 'Pending' ? 'bg-yellow-50 border-yellow-200' : 
                                latestLeave.status === 'Approved' ? 'bg-green-50 border-green-200' : 
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    ...
                                    {latestLeave.status === "Pending" && (
                                        <button 
                                            ...
                                            className="text-[9px] font-black text-red-500 
                                                hover:text-red-700 uppercase tracking-tighter 
                                    ...
                                    {(latestLeave.status === "Approved" || latestLeave.status === "Rejected") && (
                                        ...
                                        className="text-[9px] font-black text-gray-400 
                                            hover:text-[#0B2D72] uppercase underline transition-colors"
                                        >
                                            Clear
                                        </button>
                                    )}
                        ...
                        <button
                            ...
                            className="w-full bg-[#0B2D72] text-white font-black 
                                py-3.5 rounded-xl hover:bg-[#0AC4E0] hover:text-[#0B2D72] 
                                transition-all shadow-lg uppercase tracking-widest text-[10px] border-b-4 
                                border-[#0B2D72] disabled:opacity-30"
                        >
                    ...
                    <div className="bg-white w-full max-w-md rounded-3xl 
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div>
                                ...
                                <select 
                                    className="w-full bg-gray-50 border-2 border-gray-100 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" required 
                                        className="w-full bg-gray-50 border-2 rounded-xl px-4 py-3 text-sm font-bold" 
                            ...
                            <div>
                                ...
                                <textarea rows="3" 
                                    required   
                                    className="w-full bg-gray-50 border-2 rounded-xl px-4 py-3 text-sm font-bold resize-none" 
                            ...
                            <button type="submit" 
                                disabled={loading} 
                                className="w-full bg-[#0B2D72] text-white font-black py-4 
                                rounded-xl hover:bg-[#0AC4E0] uppercase tracking-widest text-[10px] 
                                border-b-4 border-[#0B2D72]">
